...
import { ChevronDown, LogOut, User, MessageCircle } from 'lucide-react';
                ...
                <div className="max-w-7xl mx-auto">
                    <ul className="flex items-center text-sm font-medium">
                        ...
                        <li className="relative">
                            ...
                            >
                                {t('nav_tenders')} <ChevronDown size={16} />
                            </button>
                            {activeDropdown === 'tenders' && (
                                <div className="absolute top-full left-0 bg-white text-gray-800 shadow-lg min-w-[200px] border border-gray-200 z-50">
                                    <Link to="/tenders" className="block px-4 py-2 hover:bg-blue-50">{t('drop_available_tenders')}</Link>
                                    <Link to="/tenders/applied" className="block px-4 py-2 hover:bg-blue-50">{t('drop_applied_tenders')}</Link>
                                    <Link to="/tenders/submit" className="block px-4 py-2 hover:bg-blue-50">{t('drop_submit_proposal')}</Link>
                                </div>
                            )}
                        </li>

                        <li>
                            <Link to="/proposals" className="block py-3.5 px-5 hover:bg-[#1565c0] transition-colors border-b-2 border-transparent hover:border-orange-400">
                                {t('nav_proposals')}
                            </Link>
                        </li>
                        ...
                        <li className="relative">
                            <button
                                ...
                                className="flex items-center gap-1 py-3.5 px-5 hover:bg-[#1565c0] transition-colors border-b-2 border-transparent hover:border-orange-400"
                            >
                                {t('nav_payments')} <ChevronDown size={16} />
                            </button>
                            {activeDropdown === 'payments' && (
                                <div className="absolute top-full left-0 bg-white text-gray-800 shadow-lg min-w-[200px] border border-gray-200 z-50">
                                    <Link to="/payments" className="block px-4 py-2 hover:bg-blue-50">{t('drop_payment_history')}</Link>
                                    <Link to="/payments/pending" className="block px-4 py-2 hover:bg-blue-50">{t('drop_pending_payments')}</Link>
                                </div>
                            )}
                        </li>
                ...
                <div className="max-w-7xl mx-auto px-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-6">
                        ...
                        <div>
                            ...
                            <ul className="space-y-2 text-sm">
                                <li><a href="#" className="hover:text-orange-400">Tender Guidelines</a></li>
                                <li><a href="#" className="hover:text-orange-400">Payment Terms</a></li>
                                <li><a href="#" className="hover:text-orange-400">Quality Standards</a></li>
                            </ul>
                        </div>
                        <div>
