...
import { ChevronDown, LogOut, User, MessageCircle, Menu, X, Home, LayoutDashboard, ClipboardList, CheckSquare, FileText, Files, HelpCircle } from 'lucide-react';
                        ...
                        <div className="hidden md:block">
                            <ul className="flex items-center text-sm font-medium">
                                ...
                                <li className="relative">
                                    <button onClick={() => setActiveDropdown(activeDropdown === 'assignments' ? null : 'assignments')} className="flex items-center gap-1 py-3.5 px-5 hover:bg-[#1565c0]">
                                        {t('nav_assignments')} <ChevronDown size={16} />
                                    </button>
                                    {activeDropdown === 'assignments' && (
                                        <div className="absolute top-full left-0 bg-white text-gray-800 shadow-lg min-w-[200px] border z-50">
                                            <Link to="/assignments" className="block px-4 py-2 hover:bg-blue-50">{t('drop_pending_assignments')}</Link>
                                            <Link to="/assignments/completed" className="block px-4 py-2 hover:bg-blue-50">{t('drop_completed_verifications')}</Link>
                                        </div>
                                    )}
                                </li>
                                <li className="relative">
                ...
                <div className="fixed inset-0 z-[60] flex">
                    ...
                    <div className="relative w-[280px] h-full bg-white shadow-2xl flex flex-col transform transition-transform duration-300 ease-in-out">
                        ...
                        <div className="bg-[#0d47a1] p-4 text-white">
                            <div className="flex items-center gap-3 mb-2">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-blue-100">{user.agency_name || 'IVA Officer'}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto py-2">
                            ...
                            <div className="px-4 py-2 text-xs font-semibold text-gray-400 uppercase tracking-wider">Assignments</div>
                            <Link to="/assignments" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-4 px-4 py-3 text-gray-700 hover:bg-gray-50">
                                <ClipboardList size={20} className="text-gray-500" />
                                <span className="font-medium">{t('drop_pending_assignments')}</span>
                            </Link>
                            <Link to="/assignments/completed" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-4 px-4 py-3 text-gray-700 hover:bg-gray-50">
                                <CheckSquare size={20} className="text-gray-500" />
                                <span className="font-medium">{t('drop_completed_verifications')}</span>
                            </Link>
